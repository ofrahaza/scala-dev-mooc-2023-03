...
object UserRepository{
    ...
    class ServiceImpl extends Service{
        ...
        def createUsers(users: List[User]): Result[List[User]] =  run(liftQuery(users).foreach(usr => query[User].insert(usr))).as(users)
        ...
        def userRoles(userId: UserId): Result[List[Role]] =  run(
            for {
                user <- userToRoleSchema.filter(_.userId == lift(userId.id))
                role <- roleSchema.join(_.code == user.roleId)
            } yield role
        )
        
        def insertRoleToUser(roleCode: RoleCode, userId: UserId): Result[Unit] = run(userToRoleSchema.insert(lift(UserToRole(roleCode.code, userId.id)))).unit
